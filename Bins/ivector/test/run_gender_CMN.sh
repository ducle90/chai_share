...
if [ $# != 1 ]; then
    ...
    echo "mean normalized (no variance normalization) at the gender level "
    echo "using statistics from the training set."
    echo ""
...
for SET in "${SETS[@]}"; do
    if [ "$SET" == "test" ]; then
        # Use statistics of the training set for normalization
        CMVN_SCP="--cmvn-scp $FEATS_DIR/train/cmvn.scp"
    else
        CMVN_SCP=""
    fi
    ...
    cp $CONFIG_PATH/utt2gender.${SET}.txt $FEATS_DIR/$SET/utt2spk
    cp $CONFIG_PATH/gender2utt.${SET}.txt $FEATS_DIR/$SET/spk2utt
    $BIN_DIR/prepare_data.sh \
        --norm-vars false $CMVN_SCP $FEATS_DIR/$SET || exit 1;
    # utt2gender was treated as utt2spk only when doing feature extraction.
    # We now swap the "fake" utt2spk with the "real" utt2spk (and spk2utt).
...
$BIN_DIR/train_dubm.sh --nj 4 $FEATS_DIR/train $NUM_GAUSS $DUBM_DIR || exit 1;
