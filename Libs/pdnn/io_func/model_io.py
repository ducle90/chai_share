       ...
       if layer.type in ['fc', 'factored', 'wfactored']:
           ...
           if layer.type == 'factored':
               for j in range(len(layer.side_Ws)):
                   ...
                   nnet_dict[dict_a] = array_2_string(layer.side_Ws[j].get_value())
               for j in range(len(layer.side_bs)):
                   dict_a = 'side_b{}_{}'.format(i, j)
                   nnet_dict[dict_a] = array_2_string(layer.side_bs[j].get_value())
...
def _file2rbm(rbm_layers, filename='rbm.in'):
    ...
    for i in range(len(rbm_layers)):
        ...
        layer.W.set_value(np.asarray(string_2_array(rbm_dict[kW]),
                                     dtype=theano.config.floatX).reshape(shape))
        # hbias
        khb = 'hbias' + str(i)
        layer.hbias.set_value(np.asarray(string_2_array(rbm_dict[khb]),
                                         dtype=theano.config.floatX))
        # vbias
        kvb = 'vbias' + str(i)
        layer.vbias.set_value(np.asarray(string_2_array(rbm_dict[kvb]),
                                         dtype=theano.config.floatX))

def _file2nnet(layers, set_layer_num = -1, filename='nnet.in', 
               factor=1.0, exc_layers=[]):
    ...
    for i in xrange(set_layer_num):
        ...
        if layer.type == 'fc' or layer.type == 'factored':
            ...
            layer.W.set_value(factor * np.asarray(string_2_array(nnet_dict[dict_a]),
                                                  dtype=theano.config.floatX).reshape(mat_shape))
            if layer.type == 'factored':
                for j in range(len(layer.side_Ws)):
                    dict_a = 'side_W{}_{}'.format(i, j)
                    mat_shape = layer.side_Ws[j].get_value().shape
                    layer.side_Ws[j].set_value(np.asarray(string_2_array(nnet_dict[dict_a]),
                                                          dtype=theano.config.floatX).reshape(mat_shape))
                for j in range(len(layer.side_bs)):
                    dict_a = 'side_b{}_{}'.format(i, j)
                    layer.side_bs[j].set_value(np.asarray(string_2_array(nnet_dict[dict_a]),
                                                          dtype=theano.config.floatX))
        elif layer.type == 'conv':
            filter_shape = layer.filter_shape
