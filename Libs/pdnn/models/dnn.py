...
from layers.mlp import HiddenLayer, FactoredHiddenLayer, WeightedFactoredHiddenLayer
...
class DNN(object):
    ...
    def __init__(self, numpy_rng, theano_rng=None,
        ...
        for i in xrange(self.hidden_layers_number):
            ...
            elif cfg.factored_cfg is None or i > 0:
                hidden_layer = HiddenLayer(rng=numpy_rng,
                                        input=layer_input,
                                        n_in=input_size,
            ...
            elif cfg.factored_cfg.type == 'factored':
                hidden_layer = FactoredHiddenLayer(
                        rng=numpy_rng, input=layer_input, n_in=input_size,
                        n_in_main=cfg.factored_cfg.n_in_main,
                        n_in_side=cfg.factored_cfg.n_in_side,
                        n_out=self.hidden_layers_sizes[i],
                        side_layers=cfg.factored_cfg.side_layers,
                        W = W, b = b, activation=self.activation)
            elif cfg.factored_cfg.type == 'wfactored':
                hidden_layer = WeightedFactoredHiddenLayer(
                        rng=numpy_rng, input=layer_input, n_in=input_size,
                        n_weights=cfg.factored_cfg.n_weights,
                        n_out=self.hidden_layers_sizes[i],
                        W = W, b = b, activation=self.activation)
            else:
                raise ValueError('Unknown factored_cfg type: {}'.format(cfg.factored_cfg.type))
        ...
        if self.n_outs > 0:
            ...
            if self.hidden_layers_number not in self.non_updated_layers:
            ...
            for i in xrange(self.hidden_layers_number):
                ...
                if type(W) != list:
                    ...
                    for j in range(len(W)):
                        self.finetune_cost += self.l1_reg * (abs(W[j]).sum())
                # Side Ws
                if cfg.factored_cfg is not None and i == 0 and \
                        cfg.factored_cfg.type == 'factored':
                    for side_W in self.layers[i].side_Ws:
                        self.finetune_cost += self.l1_reg * (abs(side_W).sum())
        ...
        if self.l2_reg is not None:
            for i in xrange(self.hidden_layers_number):
                ...
                if type(W) != list:
                    ...
                    for j in range(len(W)):
                        self.finetune_cost += self.l2_reg * T.sqr(W[j]).sum()
                # Side Ws
                if cfg.factored_cfg is not None and i == 0 and \
                        cfg.factored_cfg.type == 'factored':
                    for side_W in self.layers[i].side_Ws:
                        self.finetune_cost += self.l2_reg * T.sqr(side_W).sum()

    def build_output_function(self, shared_x):
        start = T.iscalar('start')
        end = T.iscalar('end')
        output_fn = theano.function(inputs=[start, end],
                                    outputs=self.logLayer.p_y_given_x,
                                    givens={self.x: shared_x[start:end]})
        return output_fn

    def build_validation_function(self, valid_shared_xy, batch_size):
        (valid_set_x, valid_set_y) = valid_shared_xy
        index = T.lscalar('index')
        valid_fn = theano.function(inputs=[index], outputs=self.errors,
            givens={
